...
def main():
  parser = argparse.ArgumentParser(description="Evaluate the n matrix interlingus embedding experiment",
                                   formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  parser.add_argument("--edim", "-e", default=128, type=int, help="dimension of embeddings")
  parser.add_argument("--idim", default=128, type=int, help="dimension of interlingus")
  ...
  engines = [Engine(args.edim, lshashes=[rbp], distance=CosineDistance(), vector_filters=[NearestFilter(args.nbest)]) for x in xrange(args.langs)]
  ...
  for line in infile:
    ...
    xform = np.asarray(invec*mats[inlang]*(mats[outlang].transpose()))[0]
